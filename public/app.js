...
function renderHomePage() {
    const cursosGrid = document.getElementById('cursos-recomendados-grid');
    const aulasGrid = document.getElementById('aulas-recentes-grid');
        ...
        const cardHTML = `
            <a href="detalhes.html?id=${item.id}" class="card-link"> 
                <article class="content-card">
                  <img src="${item.imagem}" alt="Thumbnail de ${item.titulo}">
                  <div class="card-info">
                    <h3>${item.titulo}</h3>
                    <p>${item.autor}</p>
        ...
        container.innerHTML = `
            <div class="detalhe-item">
                ...
                <div class="detalhe-info">
                    ...
                    <a href="index.html" class="back-to-home-btn">← Voltar para a Home</a>

                </div>
            </div>
            <div class="detalhe-outras-infos">
                <h2>O que você vai aprender</h2>
                <ul>
                    ${item.outras_infos.map(info => `<li>${info}</li>`).join('')}
                </ul>
            </div>
        `;
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('cursos-recomendados-grid')) {
        renderHomePage();
